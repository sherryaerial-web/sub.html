...
- Release authorization received. Live OA webhook verified active at existing LINE-AI Worker. Ruling: preserve existing nonextractable LINE secrets and delegate verification/push through a separately signed service binding rather than copying credentials. New bridge 4 tests cover authentication, default-off, retry identity and token acquisition crossing deadline. All 19 targeted tests pass. Both send switches remain false until binding and confirmed test.
