...
正式啟用自主練習後，既有每 5 分鐘執行的 `runCourseClosureScheduler` 會一併呼叫 `runScheduledPracticeReconciliation`。此函式會即時向 OB 讀取未來 45 天課表：OB 無法確認時整批不寫入；候補關聯的所有正式課程都消失後才補入自主練習；正式課程復活時會取消衝突的自主練習並通知參與老師。部署程式本身不會新增另一個正式觸發條件。
