...
VVIP 頁面只收集選課意願，**不代表正式保留 OB 名額**。同一位 VVIP 在當期最多可累積四堂不同的 `OB Calendar ID`；重複送出既有課程不會重複計算。會員若選錯，請聯絡管理員取消，會員端不提供自行取消。
...
3. 將 GitHub Pages 的 `vvip.html` 網址傳給 VVIP。頁面會依日期顯示下個月全部有效課程，會員選擇自己的 OB 名稱後可分次累積最多四堂。
