...
8. 確認 `老師名單` 包含可登入的老師，並在 `登入帳號` H 欄補上可教授類別、I 欄補上功能權限。正式設定為 IVY：`course_admin、payroll_admin、vvip_admin`；Sherry：`course_admin、payroll_admin`；Tako：`course_admin`。未補類別前，跨道具領取判斷不完整。
