...
- `student-practice.html`：公開學生自主練習頁，只顯示可登記空檔與匿名共用場次，不需登入老師系統。
...
- 通知設定、排程去重與社群文字保存在 Script Properties；站內收件匣另以獨立的 `通知訊息`、`通知收件人` 工作表追加保存，不會改動或覆寫既有工作表欄位與人工資料。已讀訊息保留 90 天，未讀訊息持續保留。

## 月度營運流程

通知中心會以每月「倒數第二個星期五」為一般預約基準日，使用 `Asia/Taipei` 自動推算整個月的營運節點。每月 1 日與 5 日 21:00 自動通知全部在職老師調整課程；7 日、VVIP 與一般預約節點只提醒具有 `course_admin` 權限的管理員。VVIP 學員沒有老師系統帳號，因此不由此流程通知。

請假、代課／特別課與 VVIP 的實際開關仍需管理員手動確認。通知中心可依序執行：開放請假並通知、發送請假截止提醒、結束請假、開放代課／特別課並通知、結束代課／特別課並通知。日期只是建議時間；提前操作會再次確認，但不會被寫死日期擋住。每個動作會防重複；若月度記錄與實際請假／代課開關不一致，畫面只顯示「需同步確認」，不自動覆寫正式資料。

- 代課／特別課原則上至少保留 5 天，可與 VVIP 階段重疊，但會在一般預約當週星期三前提示建議結束時間。
- 通知文字可由管理員編輯，但「全部在職老師」或「具 `course_admin` 權限的管理員」的收件規則不可改。
- 月度狀態與文字範本保存在版本化 Script Properties：`MONTHLY_OPERATIONS_V1_<YYYY_MM>` 與 `MONTHLY_OPERATIONS_TEMPLATES_V1`，不新增、搬動或改寫正式試算表欄位。
- 既有的唯一 `runCourseClosureScheduler` 每 5 分鐘觸發條件同時服務關課、月度提醒與其他已整合排程；不要再安裝第二個觸發條件。
...
學生公開空檔固定以正式課程、場地租借、老師自主練習為高優先阻擋，並保留前後 15 分鐘。學生只能在開始前 2 小時以上送出，可用時長只有 60／90／120 分鐘；已有學生場次只顯示可共用狀態，不顯示參與者。OB 學生預約與扣點目前仍由 Tako 人工處理，不另設「已扣點」欄位。

Tako 以管理員帳號登入後，可在「管理 → 自主練習」查看學生送出的資料：首次學生需依申請館別按「確認晴光資格並成立」或「確認劍潭資格並成立」，另一館仍須另外確認；也可依官方 LINE 訊息取消單一學生或協助換時間。若換到尚未確認資格的另一館，申請會轉為待確認，不會沿用原館資格。換時間會先鎖定並驗證新時段，成功後才釋出原場次；失敗時原登記與同組其他學生完全不變。

正式啟用自主練習後，既有每 5 分鐘執行的 `runCourseClosureScheduler` 會一併呼叫 `runScheduledPracticeReconciliation`。此函式只會即時向 OB 讀取未來 45 天內實際有成立、候補或待恢復自主練習的日期；沒有相關登記時不呼叫 OB。OB 無法確認時整批不寫入；候補關聯的所有正式課程都消失後才補入自主練習；正式課程復活時會取消衝突的自主練習並通知參與老師。部署程式本身不會新增另一個正式觸發條件。

### 老師自主練習內部試用

1. 先在測試用試算表執行一次 `ensureSystemStructure_()`，只會補上五張獨立的自主練習工作表；確認既有 `CourseList`、請假代課與薪資工作表列數完全不變。
2. 只用測試老師帳號登記單次、每週循環、加入共用與課程候補，並由冠蓉或 Tako 在管理頁「自主練習」查看、調整及取消。
3. 如需立即驗證，可手動執行 `runScheduledPracticeReconciliation()`；正式環境則沿用既有每 5 分鐘排程，不需另外建立重複觸發條件。
4. 回滾時停用該觸發條件並撤回前端／GAS 版本即可；五張自主練習表保持原狀供追查，不可整張清空，也不會把自主練習寫入 OB、請假代課或薪資表。
5. OB 同步失敗或回傳內容無法確認時採保守策略：不補入候補、不取消任何自主練習，管理員須先排除同步問題。
...
4. 在「VVIP 選課」頁籤的「待處理」查看每堂選課意願。管理員先在 OB 實際替該會員選好課，再逐堂按「已在 OB 選課，標記完成」；該筆會從待處理消失，但在「全部」保留並標示「已完成」。系統不會只根據 OB 課程報名人數自動認定某位會員完成選課。也可依 Email 查詢、取消單堂或依課程查看登記會員；匯出 CSV 只包含有效選課，並已防護公式注入字元。
